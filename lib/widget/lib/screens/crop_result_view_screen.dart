...
class _PickerCropResultScreenState extends State<PickerCropResultScreen> {
  ...
  void initState() {
    ...
    widget.cropStream.listen((details) async {
      // When progress is complete (1.0), update state with cropped files and selected assets.
      if (details.progress == 1.0) {
        // In version 3.x, we need to get files from data list
        List<File> files = [];
        if (details.data != null && details.data!.isNotEmpty) {
          files = details.data!
              .where((exportData) => exportData.croppedFile != null)
              .map((exportData) => exportData.croppedFile!)
              .toList();
        }
        setState(() {
          croppedFiles = files;
